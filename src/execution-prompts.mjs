...
export const buildTaskPrompt = (context, { external = false } = {}) => {
  ...
  if (external) {
    sections.push(
      ...
      '独立レビュー後に、書き込み可能なGitHub Actions工程がrepository QAを別途実行し、Coreがその結果をRelease Gateへ統合します。',
      'レビューモード自身の読み取り専用環境でQA・build・browserを実行できないことはFindingにしないでください。ソース、既存テスト、DOM、成果物から担当領域を判定してください。'
    );
  }
