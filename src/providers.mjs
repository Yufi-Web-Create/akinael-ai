...
export const createProviders = (env = process.env) => ({
  ...
  storage: {
    ...
    putObject: async ({ key, body, contentType }) => {
      ...
      const accountId = secret(env.R2_ACCOUNT_ID); const accessKeyId = secret(env.R2_ACCESS_KEY_ID); const secretAccessKey = secret(env.R2_SECRET_ACCESS_KEY); const r2Bucket = secret(env.R2_BUCKET);
